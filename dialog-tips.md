...
- To gather the information that is required to perform a task, try using a node with slots instead of a bunch of separate nodes to elicit information from users. See [Gathering information with slots](/docs/services/assistant-icp?topic=assistant-private-dialog-slots#dialog-slots).
- For a complex process flow, tell users about any information they will need to provide at the start of the process.
- Understand how the service travels through the dialog tree and the impact that folders, branches, jump-tos, and digressions have on the route. See [Dialog flow](/docs/services/assistant-icp?topic=assistant-private-dialog-overview#dialog-flow).
...
- To jump to a node in the same branch as the current node, use *Skip user input* instead of a *Jump-to*.
  ...
  This choice prevents you from having to edit the current node's settings when you remove or reorder the child nodes being jumped to. See [Defining what to do next](/docs/services/assistant-icp?topic=assistant-private-dialog-overview#jump-to).
- Before you enable digressions away from a node, test the most common user scenarios. And be sure that likely digressed-to nodes are configured to return. See [Digressions](/docs/services/assistant-icp?topic=assistant-private-dialog-runtime#digressions).
...
- Avoid using a bunch of nodes when one node will do. For example, add multiple conditional responses to a single node to return different responses depending on details provided by the user. See [Conditional responses](/docs/services/assistant-icp?topic=assistant-private-dialog-overview#multiple).
...
- Back up your workspace frequently. See [Exporting and copying workspace](/docs/services/assistant-icp?topic=assistant-private-configure-workspace#exporting-and-copying-workspaces).
...
- **Determining the number of words in user input**: You can perform any of the supported String methods on the input.text object. For example, you can find out how many words there are in a user utterance by using the following SpEL expression:
  ...
  See [Expression language methods for String](/docs/services/assistant-icp?topic=assistant-private-dialog-methods#strings) to learn about more methods you can use.
